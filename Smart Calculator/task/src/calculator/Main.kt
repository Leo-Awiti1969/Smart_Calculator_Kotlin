...
class Calculator {
    private val variables: MutableMap<String, BigDecimal> = mutableMapOf()
    ...
    private fun operation(op: String, x: BigDecimal, y: BigDecimal): BigDecimal = when (op) {
        "+" -> y + x
        "-" -> y - x
        "*" -> y * x
        "/" -> y / x
//        "^" -> y.pow(x)
    ...
    private fun String.getValue() = variables.getOrElse(this) { this.toBigDecimalOrNull() ?:
